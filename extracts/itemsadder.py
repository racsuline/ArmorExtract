...
class ItemsAdder:
    ...
    def extract(self):
        ...
        datas = [
            ...
            if (data := Utils.load_yaml(file)) is not None
        ]
        ...
        for data in datas:
            ...
            for item_id, item_data in data.get("items", {}).items():
                armor_type, layer = self.get_armor(
                    item_data.get("specific_properties", {}).get("armor", {}).get("slot") or
                    item_data.get("equipment", {}).get("slot"),
                    item_data
                )
                if not armor_type:
                    continue
                ...
                if f"{namespace}:{item_id}" not in self.item_ids.get(material, {}):
                    continue
                ...
                # Si no se encuentra, buscar por nombre del archivo
                if not texture_files:
                    texture_name = os.path.basename(texture_path)
                    texture_files = glob.glob(f"ItemsAdder/contents/**/textures/**/*{texture_name}", recursive=True)

                if not texture_files:
                    print(f"[!] No se encontró la textura: {texture_path}.png")
                    continue
                ...
                self.furnace_data["items"].setdefault(f"minecraft:{material}".lower(), {}).setdefault("custom_model_data", {})[
                    str(self.item_ids[material][item_key])
                ] = {
                    "armor_layer": {
